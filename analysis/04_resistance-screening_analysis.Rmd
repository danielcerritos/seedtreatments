...
```{r check rps1a}
rps1a.lines <- rps.final %>% 
  dplyr::select(line, Rps) %>% 
  filter(Rps == "Rps1a")

inc.incidence %>% 
  filter(line %in% rps1a.lines)
```
...
rps.final <- rps.final %>% 
  mutate(rps = case_when(
    Rps == "Rps1k" & Rps1 == "Rps 1k" ~ "Rps1k", 
    Rps == "Rps1c" & Rps1 == "Rps 1c" ~ "Rps1c", 
    line == "LD12-8677" ~ "Rps1a", 
    TRUE ~ "rps")
    )
```
...
lesion.cm <- mean.lesion %>% 
  tidy() %>% 
  filter(!line %in% c("L76-1988", "Sloan", "Williams")) %>% 
  dplyr::select(line, lesion_cm = response)
  
screening.final <- bind_cols(rps.final, mean.scores, lesion.cm) %>% 
  dplyr::select(line, rps, pr_level, mean_score, lesion_cm)

write_csv(screening.final, path = here("output", "results", 
