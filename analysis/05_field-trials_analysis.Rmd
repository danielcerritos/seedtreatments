...
The treatment effect is significant and no interaction between the seed treatment and varieties.

```{r vc means, fig.height=2, fig.width=6}
emmeans(fm.illinois.vc,  ~ treatment) %>% # increase of 12.88%
  plot(comparisons = TRUE)
```

There is a significant difference in stand at VC between the seed treatment (intego) and the untreatreated control.  
...
fm.illinois.v2 <- lmer(v2_plants.m2 ~ location*variety*treatment + (1|block:location) + (1|whole_plot), data = illinois.2017)
...
Stand is also significantly increased with the seed treatment at V2. 
...
Yield was not significantly increased with the seed treatment.
...
Location and variety by location interaction were significant factors in the three varibles measured.  
An analysis by location would be made to see if the location favors specific varieties.
...
anova(fm.mon.vc) %>% 
  knitr::kable(digits = 4, caption = "ANOVA for stand at VC in Monmouth")
anova(fm.mon.v2)  %>% 
  knitr::kable(digits = 4, caption = "ANOVA for stand at V2 in Monmouth")
anova(fm.mon.yield)  %>% 
  knitr::kable(digits = 4, caption = "ANOVA for yield in Monmouth")
```

Similar results in Monmouth as the multilocation analysis. Seed treatment increased stand at VC and V2, but  
no effect on yield. Variety is a significant factor for VC and yield, but nor for stand at V2.
...
anova(fm.orr.vc) %>% 
  knitr::kable(digits = 4, caption = "ANOVA for stand at VC in Orr")
anova(fm.orr.v2) %>% 
  knitr::kable(digits = 4, caption = "ANOVA for stand at V2 in Orr")
anova(fm.orr.yield) %>% 
  knitr::kable(digits = 4, caption = "ANOVA for yield in Orr")
```

Same as Monmouth, the seed treatment is significant for stand, but not for yield.  
Variety is significant for VC and yield.
...
anova(fm.urb.vc)%>% 
  knitr::kable(digits = 4, caption = "ANOVA for stand at VC in Urbana")
anova(fm.urb.v2) %>% 
  knitr::kable(digits = 4, caption = "ANOVA for stand at V2 in Urbana")
anova(fm.urb.yield) %>% 
  knitr::kable(digits = 4, caption = "ANOVA for yield in Monmouth")
```

Significant interaction between seed treatment and variety at VC.  
Variety is significant for VC and yield.
...
## Estimate marginal means (LS means) for fitted models

I will focus in VC and yield because variety was significant for these two variables  
and a significant interaction was observed at VC for Urbana.

### Monmouth 

Seed treatment was significant factor for stant at VC. Lets look what varieties had  
a significant variety.
```{r vc monmouth, fig.height= 9.5, fig.width=6}
emmeans(fm.mon.vc, ~ treatment|variety) %>% 
  plot(comparisons =TRUE) 
```

Only one variety (LD11-10069) had significant difference bewteen the seed treatment and the control in Monmouth.
...
No effect of seed treatment was observed for yield, but a significant effect of variety was observed.
Lets look what varieties had higher yield. No significant interaction was observed so combine  
yield means from both would be included. 
```{r yield monmouth, fig.width=11, fig.height=4}
m.mon.yield <- emmeans(fm.mon.yield, ~ variety)
m.mon.yield %>% 
  cld.emmGrid()
...
```
Varieties with no resistance gene (rps) have higher yields in Monmouth.  
Variety with low levels of partial resistance has lower yield.
