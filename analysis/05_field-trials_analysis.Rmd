...
illinois.2017$pr_level <- factor(illinois.2017$pr_level, 
                                 levels = c("High", 
                                            ...
                                            "Low"))
illinois.2017 %>% 
  ggplot(aes(x = reorder(variety, yield_kg.ha), 
...
Treatment effect significant in all variables and variety significant  
for stand at VC and yield.

### Treatment comparison 

```{r treatment urbana 18}
...
```{r vc urbana 18, fig.height= 9, fig.width=6}
...
```{r yield trt urb18, fig.height= 9, fig.width=6}
...
High level partial resistance varieties have higher yields.
...
```{r figure 3, warning=FALSE, fig.width = 7, fig.height = 7}
...
fm.iowa.v1<- lmer(v1_plants.m2 ~ variety*treatment + (1|block) + (1|whole_plot), 
                  data = iowa)
fm.iowa.r2 <- lmer(r2_plants.m2 ~ variety*treatment + (1|block) + (1|whole_plot),
                   data = iowa)
